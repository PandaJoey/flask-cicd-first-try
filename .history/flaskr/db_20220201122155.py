...
def get_db():
    if 'db' not in g:
        ...
        # pointed at by the DATABASE configuration key. This file 
        # doesn’t have to exist yet, and won’t until you initialize 
        # the database later.
    ...
    was set. If the connection exists, it is closed. Further down 
    you will tell your application about the close_db function in 
...
##might need to change this if i use a docker container       
def init_db():
    db = get_db()
    """
    open_resource() opens a file relative to the flaskr package, 
    which is useful since you wont necessarily know where that 
    location is when deploying the application later. get_db 
    returns a database connection, which is used to execute the 
